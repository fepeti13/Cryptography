...
def ECB_encrypt(original_text, key, encryption_function, padding_mode, block_size):
    ...
    if start_iterator < length:
        ...
        padded_block = padding_function(bytearray(b''), padding_mode, block_size)
        encrypted_text.extend(encryption_function(padded_block, key))
...
def _derive_encrypt_function_from_key(key_bytes):
    klen = len(key_bytes)
    if klen in (16, 24, 32):
        enc, dec = get_cipher_functions("AES")
        return enc
    elif klen == 8:
        enc, dec = get_cipher_functions("DES")
        return enc
    else:
        enc, dec = get_cipher_functions("CUSTOM")
        return enc
...
def CFB_decrypt(encrypted_text, key, decryption_function, initial_vector, block_size):
    length = len(encrypted_text)
    start_iterator = 0
    decrypted_text = bytearray()
    previous_block = initial_vector

    encryption_function = _derive_encrypt_function_from_key(key)
...
def OFB_decrypt(encrypted_text, key, decryption_function, initial_vector, block_size):
    encryption_function = _derive_encrypt_function_from_key(key)
...
def CTR_decrypt(encrypted_text, key, decryption_function, nonce, block_size):
    encryption_function = _derive_encrypt_function_from_key(key)
